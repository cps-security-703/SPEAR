"""
Enhanced evaluation metrics that measure quality over term frequency
"""
...
class EnhancedMetrics:
    """
    Enhanced metrics for evaluating RAG response quality
    Focuses on relevance, accuracy, and context usage rather than just term counting
    """
    
    @staticmethod
    def calculate_context_usage_score(response: str, context_docs: List[Dict]) -> Dict[str, any]:
        """
        Measure how well the response uses the retrieved context
        
        Args:
            response: Generated response text
            context_docs: List of context documents that were retrieved
        ...
        for doc in context_docs:
            ...
            # Check if document ID is mentioned
            if doc_id.lower() in response_lower:
                docs_referenced += 1
            
            # Check if MITRE techniques from this doc are used
            ...
            # Check if CVE ID is mentioned (for vulnerability docs)
            ...
            # Check if STRIDE categories from context are used
        ...
        # Calculate scores
        docs_referenced = min(docs_referenced, len(context_docs))  # Cap at total docs
        ...
        return {
            ...
            'context_relevance': context_usage_score * 100  # Percentage
        }
    
    @staticmethod
    def calculate_technical_depth_score(response: str) -> Dict[str, any]:
        """
        Measure technical depth and specificity (quality over quantity)
        ...
        """
        # Extract specific technical elements
        ...
        # Check for detailed explanations (sentences with technical terms)
        ...
        for sentence in sentences:
            ...
            if any(indicator in sentence_lower for indicator in technical_indicators):
                # Check if sentence is detailed (>50 chars and has specific info)
                if len(sentence) > 50:
        ...
        depth_score = 0
        depth_score += len(set(mitre_techniques)) * 10  # Unique MITRE techniques
        depth_score += len(set(cve_ids)) * 10  # Unique CVEs
        depth_score += len(cvss_scores) * 5  # CVSS scores mentioned
        depth_score += technical_sentences * 2  # Technical explanations
        depth_score += 10 if has_structure else 0  # Structured response
        depth_score += 10 if has_steps else 0  # Step-by-step guidance
    ...
    def calculate_actionability_quality(response: str) -> Dict[str, any]:
        """
        Measure quality of actionable recommendations (not just counting action words)
        
        Args:
            response: Response text
            
        Returns:
            Actionability quality metrics
        """
        # Look for specific, implementable recommendations
        specific_actions = []
        
        # Pattern: "implement/configure/enable/disable X"
        action_patterns = [
        ...
        # Check for prioritization (Critical, High, Medium, Low)
        ...
        # Check for mitigation sections
        has_mitigation_section = bool(re.search(r'mitigation|countermeasure|defense|protection', response, re.IGNORECASE))
        
        # Quality score (not just quantity)
        quality_score = 0
        quality_score += len(specific_actions) * 5  # Specific recommendations
        quality_score += 15 if has_prioritization else 0  # Prioritized recommendations
        quality_score += 10 if has_implementation else 0  # Implementation guidance
        quality_score += len(set(standards)) * 5  # Standards referenced
        quality_score += 10 if has_mitigation_section else 0  # Dedicated mitigation section
    ...
    def calculate_framework_integration_score(response: str) -> Dict[str, any]:
        """
        Measure how well the response integrates MITRE and STRIDE frameworks
        ...
        integration_score = 0
        integration_score += len(mitre_techniques) * 5  # MITRE techniques used
        integration_score += len(mitre_tactics) * 3  # MITRE tactics covered
        integration_score += len(stride_categories) * 4  # STRIDE categories used
        integration_score += 10 if mentions_mitre else 0  # Explicitly mentions MITRE
        integration_score += 10 if mentions_stride else 0  # Explicitly mentions STRIDE
        
        # Bonus for using both frameworks together
    ...
    def calculate_overall_quality_score(
        ...
        """
        Calculate comprehensive quality score combining all metrics
        ...
        if ground_truth:
            ...
            # Blend accuracy into overall score (accuracy is already 0-100, so scale it properly)
            overall_score = overall_score * 0.8 + accuracy_score * 0.2  # Fixed: was multiplying by 20 instead of 0.2
    ...
    def calculate_ground_truth_accuracy(response: str, ground_truth: Dict) -> Dict[str, any]:
        ...
        Args:
            response: Response text
            ground_truth: Expected elements
    ...
    def compare_rag_vs_non_rag(
        ...
        """
        Enhanced comparison of RAG vs non-RAG responses
        
        Args:
            rag_response: Response with RAG
            non_rag_response: Response without RAG
            rag_context_docs: Context documents used by RAG
    ...
    def generate_enhanced_report(comparison: Dict) -> str:
        """
        Generate human-readable enhanced evaluation report
