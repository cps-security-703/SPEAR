...
def collect_dqn_rewards(data, attack_type):
    ...
    for sys_key, sys_data in pre.items():
        if _system_to_attack.get(sys_key) == attack_type:
            rewards.extend(sys_data.get('dqn_episode_rewards', []))
    return np.array(rewards)

def collect_all_dqn(data):

    rewards = []
    for atk in attack_types:
        r = collect_dqn_rewards(data, atk)
        rewards.extend(r)
    return np.array(rewards)
...
print("=" * 120)
print("DQN Mean Reward Differences and Statistical Significance (SPEAR - Autonomous) via Welch's t-Test")
print("=" * 120)
...
for atk in attack_types:
    s = collect_dqn_rewards(spear, atk)
    a = collect_dqn_rewards(auto, atk)
    all_spear.append(s)
    all_auto.append(a)
...
ci_str = f"[{ci_lo:+,.1f}, {ci_hi:+,.1f}]"
print("-" * 120)
print(f"{'Overall DQN':<18} {m_s:>12.1f} {m_a:>12.1f} {diff:>+10.1f} {ci_str:>26} {t_stat:>+10.3f} {p_val:>10.4f} {sig:>5} {d:>+8.3f}")
