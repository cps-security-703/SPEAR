...
_ACN_LAYOUT = None
_ACN_PER_EVSE_KW = 7.0

def _acn_network_kw(system_id: int, node_idx: int):

    global _ACN_LAYOUT
    if _ACN_LAYOUT is None:
        try:
            from acn_network_layout import build_layout
            _ACN_LAYOUT = build_layout()
        except Exception:
            _ACN_LAYOUT = {}
    nodes = _ACN_LAYOUT.get(system_id, [])
    if nodes:
        node = nodes[int(node_idx) % len(nodes)]
        return float(node.evse_count) * _ACN_PER_EVSE_KW
    return None
...
class EVChargingStation:
    ...
    def __init__(self, evcs_id: str, bus_name: str, max_power: float, num_ports: int,
        ...
        self.voltage_ref = 240.0


        individual_ev_power = max_power / max(1, num_ports)
        self.current_ref = individual_ev_power * 1000.0 / self.voltage_ref
        self.power_ref = individual_ev_power

        self.rated_power_per_port = individual_ev_power


        self.current_setpoint = self.current_ref
        self.voltage_setpoint = 1.0
        self.voltage_reference = 240.0
        self.current_reference = self.current_ref
        self.power_reference = individual_ev_power
        ...
        self.min_disconnect_time = 0.05
        self.connection_probability = 0.0
        self.initialization_time = 0.0
        self.auto_connect_delay = 0.04
    ...
    def update_recovery(self, current_time: float):
        ...
        recovery_time_constant = 120.0


        if int(recovery_duration) % 10 == 0 and recovery_duration > 0:
        ...
        if recovery_duration >= recovery_time_constant:
            ...
            if self.pre_attack_metrics:
                ...
                decay_factor = 1.0 - np.exp(-3.0 * recovery_factor)
    ...
    def _force_ev_connection(self, current_time_hours: float):
        ...
        if 7 <= current_time_hours <= 9 or 17 <= current_time_hours <= 21:
            ...
            self.soc = np.random.uniform(0.15, 0.35)
        else:

            self.soc = np.random.uniform(0.2, 0.5)
        ...
        estimated_energy = soc_deficit * self.params.capacity
        estimated_duration = estimated_energy / max(self.power_reference, 0.1)
    ...
    def disconnect_ev(self, current_time_hours: float):
        ...
        bg_ports = max(0, int(self.num_ports * 0.50))
    ...
    def check_ev_status(self, current_time_hours: float):
        ...
        if self.soc >= 0.80 and charging_duration >= 0.15:
            should_disconnect = True
            disconnect_reason = f"SOC {self.soc:.2f} >= 80% (CC-CV knee) after {charging_duration:.2f}h"


        if self.emergency_mode and self.soc > 0.7:
            should_disconnect = True
    ...
    def get_availability_factor(self, time_hours: float) -> float:
        ...
        if 7 <= time_hours <= 9:
            return 0.9
        elif 17 <= time_hours <= 21:
            return 0.95
        elif 9 <= time_hours <= 17:
            return 0.6
        elif 21 <= time_hours <= 23:
            return 0.7
        else:
            return 0.3
    ...
    def update_dynamics(self, grid_voltage_rms: float, dt: float, current_time_hours: float,
        ...
        if self.ev_connected:
            ...
            _station_power_cap = getattr(self, 'max_power', 192.0) * 2.0
            ...
            self.voltage_setpoint = self.voltage_measured / 240.0
            ...
            if self.charging_sessions and self.current_ev_id:
                for session in self.charging_sessions:
                    if session.get('customer_id') == f"cust_{self.evcs_id}_{self.current_ev_id}":
                        energy_increment = self.power_measured * dt / 3600.0
            ...
            background_current = background_power * 1000.0 / 240.0
    ...
    def start_charging_session(self, customer_id: str, start_time: float,
        ...
        if self.available_ports > 0:
            session = {
                ...
                'energy_delivered': power_level * duration / 60.0,
    ...
    def update_customer_wait_times(self, current_time: float):
        ...
        if time_since_last_arrival >= arrival_interval:
            ...
            new_customer = {
                ...
                'requested_charge': (soc_target - soc_initial) * 60.0
            }
    ...
    def _calculate_real_charging_time(self, current_time: float = None) -> float:
        ...
        target_soc = 0.80
        energy_needed = battery_capacity * (target_soc - arrival_soc)
        ...
        if ambient_temp > 35.0:
            derate_pct = (ambient_temp - 35.0) * 0.02
            effective_power *= (1.0 - min(derate_pct, 0.20))


        utilization = 1.0 - (getattr(self, 'available_ports', 0) / max(1, getattr(self, 'num_ports', 1)))
        transformer_drop = utilization * 0.03
        effective_power *= (1.0 - transformer_drop)


        cv_fraction = max(0.0, (arrival_soc - 0.20) / 0.60) * 0.30
        avg_power_factor = 1.0 - cv_fraction * 0.40
        ...
        effective_power = max(1.0, effective_power)
        ...
        if hasattr(self, 'grid_frequency') and self.grid_frequency > 0:
            ...
            attack_time_factor += freq_deviation * 0.3
    ...
    def _calculate_real_queue_length(self, current_time: float = None) -> int:
        ...
        if hasattr(self, 'attack_active') and self.attack_active:
            ...
            rated_power = getattr(self, 'rated_power_per_port', 7.68)
            normal_charging_time = (36.0 / max(1.0, rated_power)) * 60.0

            if current_charging_time > normal_charging_time:

                delay_minutes = current_charging_time - normal_charging_time

                attack_queue_factor = int(delay_minutes * 0.5)


                self.accumulated_queue_delay += delay_minutes * time_delta / 60.0
            ...
            if hasattr(self, 'accumulated_queue_delay') and self.accumulated_queue_delay > 0:
                ...
                attack_queue_factor = int(self.accumulated_queue_delay * 2.0)
        ...
        if hasattr(self, 'available_ports') and self.available_ports < self.num_ports:
            ...
            if port_utilization > 0.9:
                utilization_factor = int((port_utilization - 0.9) * 30)
...
class EnhancedChargingManagementSystem:
    ...
    def __init__(self, stations: List[EVChargingStation] = None, use_pinn: bool = True, system_id: int = None):
        ...
        self.total_power_limit = 430.0


        self.voltage_droop_gain = 2.5
        self.frequency_droop_gain = 4.1
        self.soc_weight = 0.3
        ...
        self.rate_change_limit = 0.5


        self.consecutive_anomaly_limit = 1
        ...
        if self.research_mode:
            ...
            self.max_power_reference = 7.68
            self.max_voltage_reference = 240.0
            self.max_current_reference = 32.0
            self.rate_change_limit = 2.0
        else:
    ...
    def _initialize_pinn_optimizer(self):
        ...
        try:
            ...
            model_paths = [
                'pinn_evcs_optimizer_pretrained.pth',
                'pinn_evcs_optimizer.pth',
                'pinn_optimizer.pth'
            ]
    ...
    def _optimize_with_pinn(self, station_id: int, current_time: float,
        ...
        if self.federated_manager is not None:
            try:
                ...
                nominal_current = max(1.0, min(station.soc * 26.0 + 6.0, 32.0))
                ...
                station_utilization = demand_factor
                queue_len = getattr(station, 'queue_length', max(0, int(demand_factor * 8)))
                time_of_day = (current_time / 3600.0) % 24.0
                ...
                if current_ref > 32.0:
                    current_ref = min(32.0, power_ref * 1000.0 / voltage_ref)
    ...
    def _apply_input_attacks(self, station_data: Dict, station_id: int, current_time: float) -> Dict:
        ...
        stealth_level = float(self.attack_params.get('stealth_level', 0.0))
        magnitude = float(magnitude) * max(0.05, 1.0 - 0.75 * stealth_level)
    ...
    def _security_validation(self, station_id: int, voltage_ref: float, current_ref: float,
        ...
        heuristic_anomaly = (anomaly_detected or input_anomaly) and ml_anomaly
        if heuristic_anomaly or ml_anomaly:
            self.anomaly_counters[station_id] += 1
            ...
            if self.anomaly_counters[station_id] >= self.consecutive_anomaly_limit:
                ...
                return voltage_ref, current_ref, power_ref, False
        else:
    ...
    def _optimize_heuristic(self, station_id: int, current_time: float,
        ...
        min_station_power = station.max_power * 0.20
        ...
        if station.ev_connected:
            ...
            if station.soc < 0.2:
                base_power = station.max_power * 0.80
                target_voltage = 240.0
                target_current = 32.0
            elif station.soc < 0.8:
                base_power = station.max_power * 0.60
                target_voltage = 240.0
                target_current = 24.0
            else:
                base_power = station.max_power * 0.30
                target_voltage = 240.0
                target_current = 16.0
    ...
    def generate_daily_charging_profile(self, time_hours: float) -> float:
        ...
        base_load_level = 0.20
        total_load = base_load_level


        night_center = 2.5
        if time_hours >= 23:
            night_hour = time_hours - 24
        else:
        ...
        if 23 <= time_hours or time_hours <= 6:
            total_load *= 1.05


        return np.clip(total_load, 0.20, 1.4)
    ...
    def ensure_evcs_initialization(self):
        ...
        for station in self.stations:
            if not station.ev_connected:
                ...
                station.initialization_time = 0.0
                station.connect_new_ev(0.0)
        ...
        for station in self.stations:
            if station.ev_connected:
                ...
                station.set_references(240.0, 16.0, 3.84)
...
class CentralChargingCoordinator:
    ...
    def __init__(self):
        ...
        self.max_queue_time = 30.0
        self.load_balance_threshold = 0.2
        self.emergency_power_reduction = 0.3


        self.customer_arrival_rate = 0.10
        self.charging_demand_distribution = {
            'low': {'min': 10, 'max': 30, 'probability': 0.4},
            'medium': {'min': 30, 'max': 60, 'probability': 0.4},
            'high': {'min': 60, 'max': 100, 'probability': 0.2}
        }


        self.initial_customers_per_station = 2
        self.initial_charging_sessions = True
        ...
        for sys_id, system in self.distribution_systems.items():
            if hasattr(system, 'ev_stations'):
                for station_idx, station in enumerate(system.ev_stations):
                    ...
                    arrival_probability = self.customer_arrival_rate * (10.0 / 60.0)


                    time_scaling = 1.0 + (current_time / 3600.0) * 0.5
            ...
            if hasattr(system, 'ev_stations'):
                for station_idx, station in enumerate(system.ev_stations):
                    ...
                    for i in range(self.initial_customers_per_station):
                        ...
                        arrival_time = -np.random.uniform(5.0, 15.0)
                ...
                for station_idx, station in enumerate(system.ev_stations):
                    ...
                    if station.available_ports > 0 and station.customer_queue:
                        ...
                        if station.current_setpoint > 0:
                            charging_duration = (customer['requested_charge'] / station.current_setpoint) * 60.0
                        else:

                            charging_duration = 45.0
                            if station.ev_connected:
    ...
    def _calculate_customer_satisfaction_impact(self, time_impact_factor: float) -> float:
        ...
        return max(0.0, min(1.0, satisfaction_impact))
    ...
    def _calculate_charging_time_impact(self, load_change_mw):
        ...
        if load_change_mw < 0:
            ...
            impact_factor = 1.0 + (abs(load_change_mw) / 1000.0)
            new_charging_time = base_charging_time * impact_factor
        else:

            impact_factor = 1.0 - (load_change_mw / 2000.0)
            new_charging_time = base_charging_time * impact_factor
    ...
    def _calculate_queue_impact(self, load_change_mw, system_id):
        ...
        if load_change_mw < 0:
            ...
            return abs(load_change_mw) * 0.5
        else:

            return -load_change_mw * 0.3
    ...
    def _balance_load(self, overloaded_systems, underloaded_systems):
        ...
        for overloaded_sys in overloaded_systems:
            for underloaded_sys in underloaded_systems:
                ...
                transfer_amount = min(overloaded_load * 0.1,
...
class IEEE14BusAGC:
    ...
    def __init__(self):
        ...
        self.beta = 21.5
        self.tie_line_power = 0.0
        self.tie_line_scheduled = 0.0
        self.ACE = 0.0
        ...
        self.P_load_base = self.net.load.p_mw.sum()
        self.P_load_current = self.P_load_base
        self.P_dist_total = 0.0


        self.P_dist_scheduled = 0.0
        self.dist_forecast_tau = 600.0
        ...
        self.ref_blend = 0.5
        self.P_gen_total = 0.0


        self.load_profile_times = None
        self.load_profile_multipliers = None
        self.base_load_nominal = self.P_load_base
        self.dist_base_loads = {}
        self.reference_power = self.P_load_base


        self.agc_time_step = 4.0
    ...
    def _create_ieee14_bus_network(self):
        ...
        gen_data = [
            [1, 0.0, 0.0, 1.06, 0.0, 332.4, 10.0],
            [2, 40.0, 0.0, 1.045, 0.0, 140.0, 10.0],
            [3, 0.0, 0.0, 1.01, 0.0, 100.0, 10.0],
            [6, 0.0, 0.0, 1.07, 0.0, 100.0, 10.0],
            [8, 0.0, 0.0, 1.09, 0.0, 100.0, 10.0]
        ]
    ...
    def agc_dynamics(self, t, x):
        ...
        delta_PL = (total_load - self.reference_power) / self.reference_power
    ...
    def update_frequency(self, dt=0.1):
        ...
        if sol.success:
            ...
            if all(np.isfinite(x_new)):
                ...
                self.delta_f = float(np.clip(np.nan_to_num(x_new[0], nan=0.0),
                                             -2.0 / self.f_nominal, 2.0 / self.f_nominal))
            ...
            self.delta_f = -power_imbalance * 0.1


        _DF_PU_LIMIT = 2.0 / self.f_nominal
        self.delta_f = float(np.clip(self.delta_f, -_DF_PU_LIMIT, _DF_PU_LIMIT))
        self.frequency = self.f_nominal * (1.0 + self.delta_f)
    ...
    def update_agc_reference(self, current_time: float):
        ...
        if current_time - self.last_agc_update >= self.agc_time_step:
            ...
            self.agc_integral += -self.agc_Ki * ace * self.agc_time_step

            _int_lim = 0.5 * max(self.reference_power, 1.0)
            self.agc_integral = float(np.clip(self.agc_integral, -_int_lim, _int_lim))

            ace_adjustment = self.agc_integral - (ace / self.beta if self.beta != 0 else 0.0)


            if self.P_dist_scheduled <= 0.0:
                self.P_dist_scheduled = self.P_dist_total
            _alpha = min(1.0, self.agc_time_step / max(self.dist_forecast_tau, 1.0))
            self.P_dist_scheduled += _alpha * (self.P_dist_total - self.P_dist_scheduled)


            base_reference = scaled_base_load + self.P_dist_scheduled


            new_reference = base_reference + ace_adjustment


            _wn = getattr(self, 'ref_blend', 0.3)
            self.reference_power = (1.0 - _wn) * self.reference_power + _wn * new_reference
    ...
    def update_distribution_load(self, dist_loads: Dict[int, float]):
        ...
        if any(load > 1000 for load in dist_loads.values()):
...
class OpenDSSInterface:
        ...
        try:
            ...
            for i, station in enumerate(self.ev_stations):
                ...
                self.dss.Command(
                    ...
                    f"Phases=3 kV={self.bus_kv_ll(station.bus_name)} "
                    f"kW={cached_kw} kvar={cached_kvar} Model=1"
                )
            ...
            for pv in self.pv_systems:
                ...
                self.dss.Command(
                    ...
                    f"kV={self.bus_kv_ll(bus)} kVA={kva} Pmpp={pmpp} pf=1"
                )
            ...
            for stor in self.storage_devices:
                ...
                self.dss.Command(
                    ...
                    f"kV={self.bus_kv_ll(bus)} kWRated={kwrated} kWhRated={kwhrated} "
    ...
    def bus_kv_ll(self, bus_name: str, default: float = 4.16) -> float:

        try:
            self.circuit.SetActiveBus(bus_name.split('.')[0])
            kv_ln = self.dss.Bus.kVBase()
            if kv_ln and kv_ln > 0:
                return round(kv_ln * math.sqrt(3), 4)
        except Exception:
            pass
        return default
    ...
    def add_distributed_energy_resources(
        ...
        for idx, config in enumerate(pv_configs):
            ...
            kv_level = config.get('kv') or self.bus_kv_ll(bus_name)
    ...
    def update_ev_loads(self, current_time: float = 0.0, system_frequency: float = 60.0):
        ...
        if not self._path_logged:
            if acn_live:
                ...
                print(f"[DS{self.system_id}] ℹ  Legacy EVCSController dynamics path "
        ...
        for i, station in enumerate(self.ev_stations):
            try:
                ...
                grid_voltage_rms = grid_voltage_pu * 12470
                ...
                try:
                    ...
                    power = self.dss.CktElement.Powers()[0]
                    ...
                    self._evcs_load_cache[load_name] = actual_power
                ...
                if hasattr(station, 'attack_active') and station.attack_active:
                    ...
                    _grid_kw = manipulated_power
                    _ap = getattr(self.cms, 'attack_params', {}) if hasattr(self, 'cms') and self.cms else {}
                    if _ap.get('target_nodes'):
                        _net_kw = _acn_network_kw(self.system_id, i)
                        if _net_kw:
                            _grid_kw = _net_kw * station.attack_manipulation_factor


                            _rated_kw = _net_kw * (ACN_P_MAX_KW / _ACN_PER_EVSE_KW)
                            _grid_kw = max(0.0, min(_grid_kw, _rated_kw))
                    ...
                    try:
                        ...
                        manipulated_kw = _grid_kw
                        manipulated_kvar = _grid_kw * 0.1


                        self.dss.Command(f"Load.{load_name}.kW={manipulated_kw:.2f}")
                        self.dss.Command(f"Load.{load_name}.kvar={manipulated_kvar:.2f}")
                        self._evcs_load_cache[load_name] = manipulated_kw

                        print(f" Attack Load Update: System {self.system_id}, Station/Node {i}, "
                              f"Original: {dynamics_result['total_power']:.2f}kW, "
                              f"Grid load: {manipulated_kw:.2f}kW, "
                    ...
                    if hasattr(station, 'attack_active') and not station.attack_active:
                        ...
                        if hasattr(station, 'dynamics_result_cache'):
                            ...
                            if station.ev_connected:
                                ...
                                charging_power_per_ev = 50.0
                                active_ports = getattr(station, 'num_ports', 16) - getattr(station, 'available_ports', 0)
                                active_ports = max(1, active_ports)
                                station.current_load = charging_power_per_ev * active_ports
                            else:

                                station.current_load = 0.001


                            try:
                                load_name = f"EVCS_{self.system_id}_{i:02d}"
                                normal_kw = station.current_load
                                ...
                                self._evcs_load_cache[load_name] = normal_kw
                                ...
                                if station.recovery_message_counter % 50 == 0:
        ...
        try:
            ...
            if voltage_violations or overloaded_lines:
                ...
                if voltage_violations:
                    ...
                    for bus, v_pu, vtype in voltage_violations[:3]:
                ...
                if overloaded_lines:
                    ...
                    for line, power, rating in overloaded_lines[:3]:
    ...
    def get_total_load(self) -> float:
        ...
        try:
            ...
            storage_charge_kw = sum(max(0.0, -p) for p in self.current_storage_power.values()) if self.current_storage_power else 0.0
            der_adjustment_kw = storage_charge_kw - storage_discharge_kw - pv_kw
            total_system_load = (total_load + ev_load + der_adjustment_kw) / 1000
    ...
    def launch_cyber_attack(self, attack_config: Dict):
        ...
        target_percentage = attack_config.get('target_percentage', 100)
        ...
        target_stations = list(range(num_target_stations))
...
class HierarchicalCoSimulation:
    ...
    def __init__(self, realtime_rl_controller=None, use_dqn_sac_security=True, use_enhanced_pinn=True):
        ...
        self.results = {
            ...
            'agc_updates': [],
            'charging_time_data': {},
            'charging_time_timestamps': {},
            'queue_management_data': {},
            'queue_management_timestamps': {},
            'scheduling_data': {},
            'customer_satisfaction_data': {},
            'customer_satisfaction_timestamps': {},
            'utilization_data': {},
        ...
        self.dist_dt = 1.0
        self.agc_dt = 4.0
        self.coordination_dt = 4.0
        self.customer_arrival_dt = 60
        self.total_duration = 7200.0


        self.ev_penetration_scale = 10.0


        self._attack_load_lag = {}
        self.carry_forward_tau = 45.0
    ...
    def _load_enhanced_pinn_models(self):
        ...
        try:
            ...
            _n_ds = len(_glob.glob('federated_pinn_system_*.pth')) or 6
            fed_config = FederatedPINNConfig()
            fed_config.num_distribution_systems = _n_ds


            for sys_id in range(1, _n_ds + 1):
        ...
        for sys_id, dist_info in self.distribution_systems.items():
            ...
            if not hasattr(dist_sys, 'ev_stations') or len(dist_sys.ev_stations) == 0:
                ...
                system_specific_configs = {
                    ...
                    1: [
                        {'bus': '890', 'max_power': round(25 * _L2, 1), 'num_ports': 25},
                        {'bus': '844', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '860', 'max_power': round(4  * _L2, 1), 'num_ports': 4},
                        {'bus': '840', 'max_power': round(10 * _L2, 1), 'num_ports': 10},
                        {'bus': '848', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '830', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '848', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '830', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '824', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '826', 'max_power': round(4  * _L2, 1), 'num_ports': 4},
                    ],
                    ...
                    7: [
                        {'bus': '890', 'max_power': round(20 * _L2, 1), 'num_ports': 20},
                        {'bus': '844', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '860', 'max_power': round(3  * _L2, 1), 'num_ports': 3},
                        {'bus': '840', 'max_power': round(8  * _L2, 1), 'num_ports': 8},
                        {'bus': '848', 'max_power': round(4  * _L2, 1), 'num_ports': 4},
                        {'bus': '830', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '848', 'max_power': round(4  * _L2, 1), 'num_ports': 4},
                        {'bus': '830', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '824', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '826', 'max_power': round(3  * _L2, 1), 'num_ports': 3},
                    ],

                    8: [
                        {'bus': '890', 'max_power': round(30 * _L2, 1), 'num_ports': 30},
                        {'bus': '844', 'max_power': round(8  * _L2, 1), 'num_ports': 8},
                        {'bus': '860', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '840', 'max_power': round(12 * _L2, 1), 'num_ports': 12},
                        {'bus': '848', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '830', 'max_power': round(7  * _L2, 1), 'num_ports': 7},
                        {'bus': '848', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '830', 'max_power': round(7  * _L2, 1), 'num_ports': 7},
                        {'bus': '824', 'max_power': round(8  * _L2, 1), 'num_ports': 8},
                        {'bus': '826', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                    ],

                    9: [
                        {'bus': '890', 'max_power': round(28 * _L2, 1), 'num_ports': 28},
                        {'bus': '844', 'max_power': round(7  * _L2, 1), 'num_ports': 7},
                        {'bus': '860', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '840', 'max_power': round(11 * _L2, 1), 'num_ports': 11},
                        {'bus': '848', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '830', 'max_power': round(7  * _L2, 1), 'num_ports': 7},
                        {'bus': '848', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '830', 'max_power': round(7  * _L2, 1), 'num_ports': 7},
                        {'bus': '824', 'max_power': round(7  * _L2, 1), 'num_ports': 7},
                        {'bus': '826', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                    ],

                    10: [
                        {'bus': '890', 'max_power': round(22 * _L2, 1), 'num_ports': 22},
                        {'bus': '844', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '860', 'max_power': round(4  * _L2, 1), 'num_ports': 4},
                        {'bus': '840', 'max_power': round(9  * _L2, 1), 'num_ports': 9},
                        {'bus': '848', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '830', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                        {'bus': '848', 'max_power': round(5  * _L2, 1), 'num_ports': 5},
                        {'bus': '830', 'max_power': round(6  * _L2, 1), 'num_ports': 6},
                ...
                base_configs = system_specific_configs.get(sys_id, system_specific_configs[1])
                ...
                _ds_labels = ['Urban Area', 'Highway Corridor', 'Mixed Area', 'Industrial Zone',
                              'Commercial District', 'Residential Complex', 'Rural Feeder',
                              'Transit Hub', 'Port / Logistics', 'University Campus']
                _label = _ds_labels[(sys_id - 1) % len(_ds_labels)]
                print(f"    Using {_label} configuration")
                    ...
                    try:
                        ...
                        try:
                            dist_sys.dss.Command(f"New Load.{load_name} "
                                               f"Bus1={station.bus_name} "
                                               f"Phases=3 "
                                               f"kV={dist_sys.bus_kv_ll(station.bus_name)} "
                                               f"kW=0.1 "
                                               f"kvar=0.02 "
                                               f"Model=1")
    ...
    def _plot_with_discontinuity_handling(self, ax, time_data, y_data, **plot_kwargs):
        ...
        if len(time_data) > 1:
            ...
            jump_indices = np.where(time_diffs < 0)[0]
    ...
    def run_hierarchical_simulation(self, attack_scenarios: List[Dict] = None, max_wall_time_sec: float = None):
        ...
        self.current_attack_scenarios = attack_scenarios
        ...
        for step in range(dist_steps):
            ...
            if self.realtime_rl_controller:
                ...
                for target_sys, attack_params in attack_decisions.items():
                    if target_sys in self.distribution_systems:
                        ...
                        if hasattr(dist_sys, 'cms') and dist_sys.cms:
                            ...
                            n_st = len(dist_sys.ev_stations)
                            _target_node = attack_params.get('target_node')
                            _target_nodes = attack_params.get('target_nodes')


                            if _target_nodes:
                                target_station_ids = sorted({int(x) % n_st for x in _target_nodes})
                            else:
                                target_station_ids = list(range(min(n_st,
                                                            int(n_st * attack_params.get('target_percentage', 100) / 100))))
                            dist_sys.cms.attack_params = {
                                ...
                                'stealth_level': attack_params.get('stealth_level', 0.0),
                ...
                if self.simulation_time % 30.0 == 0:
                        ...
                        if not attack.get('active', False):
                            ...
                            if target_sys in self.distribution_systems:
                                ...
                                if hasattr(dist_sys, 'cms') and dist_sys.cms:
                                    ...
                                    dist_sys.cms.rl_attack_impact = rl_attack_impact
                                    dist_sys.cms.attack_params = {
                                        'type': attack['type'],
                                        'magnitude': attack['magnitude'],
                                        'targets': list(range(len(dist_sys.ev_stations))),
                                        'start_time': attack['start_time'],
                                        'duration': attack['duration'],
                                        'stealth_level': attack.get('stealth_level', 0.0),
                                        'rl_impact': rl_attack_impact
                                    }
            ...
            for sys_id, dist_info in self.distribution_systems.items():
                            ...
                            if hasattr(station, 'evcs_controller'):
                                ...
                                try:
                                    ...
                                    if hasattr(station.evcs_controller, '_update_dynamics_euler'):
                                        try:
                                            grid_voltage_v = grid_voltage * 7200.0
                                            dt_simulation = 1.0
                ...
                _pen_scale = getattr(self, 'ev_penetration_scale', 1.0)
                _raw_variation = (load - transmission_base_load) * _pen_scale


                _tau_c = getattr(self, 'carry_forward_tau', 0.0)
                if _tau_c > 0.0:
                    _prev = self._attack_load_lag.get(sys_id, _raw_variation)
                    _beta_c = self.dist_dt / (_tau_c + self.dist_dt)
                    _carried = _prev + _beta_c * (_raw_variation - _prev)
                    self._attack_load_lag[sys_id] = _carried
                    evcs_variation = _carried
                else:
                    evcs_variation = _raw_variation
                        ...
                        if station_count > 0:
                            ...
                            for station in dist_sys.ev_stations:
                                ...
                                try:
                                    ...
                                    bus_voltage_pu = dist_sys.dss.Bus.puVmagAngle()[0]
                                ...
                                if station_power < 1.0 or station_power > _station_max_kw:
                                    rated_power_per_port = getattr(station, 'rated_power_per_port', ACN_P_MAX_KW)

                                    voltage_pu = station_voltage / ACN_EVSE_VOLTAGE_V
                                ...
                                if attack_scenarios and hasattr(station, 'attack_active') and station.attack_active:
                                    ...
                                    if hasattr(station, 'attack_type') and hasattr(station, 'rl_attack_impact'):
                                        ...
                                        if _st_atype == 'power_disruption':
                                            ...
                                            station_voltage = max(216.0, station_voltage - voltage_drop)
                                            ...
                                            station_voltage  = max(200.0, station_voltage - voltage_drop)
                                ...
                                station_voltage = float(np.clip(station_voltage, 0.0, 1000.0))
                                _p_max_kw = getattr(station, 'num_ports', 10) * ACN_P_MAX_KW * 1.1
                ...
                if hasattr(self, 'current_attack_scenarios') and self.current_attack_scenarios:
                    for attack in self.current_attack_scenarios:
                        if (attack['start_time'] <= self.simulation_time <=
                            ...
                            impact['load_change_mw'] = 0.0
                            impact['load_change_percent'] = 0.0
    ...
    def _generate_rl_attack_impact(self, system_id: int, attack_type: str, current_time: float) -> Dict:
        ...
        if system_id not in self.rl_attack_systems:
            ...
            return {
                'power_efficiency_reduction': 0.6,
                'load_instability_factor': 2.5,
                'voltage_efficiency_loss': 0.7,
                'demand_instability_factor': 2.3,
                'frequency_efficiency_loss': 0.5
            }
        ...
        try:
            ...
            if rl_attack:
                ...
                base_impact = min(0.8, magnitude / 50.0)
                duration_factor = min(1.5, duration / 60.0)
                stealth_factor = 1.0 - stealth_score * 0.3
    ...
    def plot_hierarchical_results_old(self):
        ...
        if self.results['bus_voltages']:
            ...
            for i in range(min(5, bus_voltages.shape[1])):
    ...
    def _plot_evcs_voltage_analysis(self):
        ...
        for idx, sys_id in enumerate(range(1, 7)):
            if sys_id in self.results['evcs_voltage_data'] and len(self.results['evcs_voltage_data'][sys_id]) > 0:
                voltage_data_dict = self.results['evcs_voltage_data'][sys_id]
    ...
    def _plot_evcs_power_analysis(self):
        ...
        for idx, sys_id in enumerate(range(1, 7)):
            if sys_id in self.results['evcs_power_data'] and len(self.results['evcs_power_data'][sys_id]) > 0:
                power_data_dict = self.results['evcs_power_data'][sys_id]
    ...
    def _plot_evcs_current_analysis(self):
        ...
        for idx, sys_id in enumerate(range(1, 7)):
            if sys_id in self.results['evcs_current_data'] and len(self.results['evcs_current_data'][sys_id]) > 0:
                current_data_dict = self.results['evcs_current_data'][sys_id]
    ...
    def get_simulation_statistics(self) -> Dict:
        ...
        if all_charging_times:
            stats['global_charging_metrics'] = {
                ...
                'charging_time_efficiency': 45.0 / np.mean(all_charging_times),
